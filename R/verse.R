core <- c("sorensonimpact", "conflicted", "janitor", "fs", "naniar", "visdat", "lubridate", "skimr", "readxl")
...
siverse_attach <- function() {
  ...
  msg(
    cli::rule(
      ...
      right = paste0("siverse ", package_version("siverse"))
    ),
    startup = TRUE
  )
...
#' Conflicts between the siverse and other packages
#'
...
#' @examples
#' siverse_conflicts()
siverse_conflicts <- function() {
  envs <- purrr::set_names(search())
  objs <- invert(lapply(envs, ls_env))

  conflicts <- purrr::keep(objs, ~ length(.x) > 1)

  pkg_names <- paste0("package:", siverse_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% pkg_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "siverse_conflicts")
}

siverse_conflict_message <- function(x) {
  if (length(x) == 0) return("")

  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "siverse_conflicts()"
  )
...
#' @export
print.siverse_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(siverse_conflict_message(x))
}
...
#' Update siverse packages
#'
#' This will check to see if all siverse packages (and optionally, their
...
#' siverse_update()
#' }
siverse_update <- function(recursive = FALSE) {

  deps <- siverse_deps(recursive)
  behind <- dplyr::filter(deps, behind)

  if (nrow(behind) == 0) {
    cli::cat_line("All siverse packages up-to-date")
    return(invisible())
  }
...
#' List all siverse dependencies
#'
#' @param recursive If \code{TRUE}, will also list all dependencies of
#'   siverse packages.
#' @export
siverse_deps <- function(recursive = FALSE) {
  pkgs <- utils::available.packages()
  deps <- tools::package_dependencies("siverse", pkgs, recursive = recursive)
...
#' List all packages in the siverse
#'
#' @param include_self Include siverse in the list?
#' @export
#' @examples
#' siverse_packages()
siverse_packages <- function(include_self = TRUE) {
  raw <- utils::packageDescription("siverse")$Imports
  imports <- strsplit(raw, ",")[[1]]
  parsed <- gsub("^\\s+|\\s+$", "", imports)
  names <- vapply(strsplit(parsed, "\\s+"), "[[", 1, FUN.VALUE = character(1))

  if (include_self) {
    names <- c(names, "siverse")
  }
